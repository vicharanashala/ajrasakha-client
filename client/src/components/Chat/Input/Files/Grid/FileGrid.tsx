...
// How many tiles to render at once. Keeps the DOM bounded regardless of how many files the
// user has — more are revealed a page at a time via the "Load more" button below, rather
// than rendering the entire (already-fetched) list in one shot.
const PAGE_SIZE = 30;
...
export default function FileGrid({ files }: { files: TFile[] }) {
  ...
  // Start each new search (or the initial list) back at one page, so results aren't
  // confusingly cut off at whatever count was reached before the search changed.
  useEffect(() => {
            ...
            {hasMore && (
              <div className="mt-3 flex justify-center">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-border-heavy bg-surface-secondary px-4 py-2 text-sm font-medium text-text-primary transition-colors hover:bg-surface-hover"
                >
                  {localize('com_files_load_more')}
                </button>
              </div>
            )}
          </>
